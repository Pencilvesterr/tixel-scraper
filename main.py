...
def lambda_handler(event, context):
    ...
    try:
        ...
        for category, events in all_data.items():
            filename = f"events/{timestamp}/{category}.json"
            logger.debug(f"Uploading {len(events)} events to {filename}")
            s3.upload_file(filename, json.dumps(events).encode())
